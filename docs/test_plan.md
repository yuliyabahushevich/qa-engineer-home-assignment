Test plan
ID# 1 - Placing a single bet on the platform and validating the bet placement and the balance update.
Priority: Critical
...
ID# 2 - Successfully place a bet with the maximum allowed stake (€100.00)
Priority: High
...
ID#3 - Prevent bet placement when stake exceeds available balance
Priority: Critical
...
ID#4 - Validate invalid stake values and error messages
Priority: Medium
...
ID#5 - Selecting another odds option replaces previous selection
Priority: Medium
